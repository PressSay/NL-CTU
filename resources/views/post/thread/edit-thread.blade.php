<x-app-layout :title="'Post Thread'" :threadPost="1" :noFooter="1">
   ...
   <div class="block my-7  md:my-4">
    <div class="text-sm breadcrumbs hidden sm:block">
      <ul>
        ...
        <li><a href="/post/category-for-thread">{{ $thread->category->category->title }}</a></li>
    ...
    <div class="flex sm:hidden">
      <a href="/post/category-for-thread" class="flex items-center">
        ...
        <p class="font-semibold w-32 truncate">{{ $thread->category->title }}</p>
      </a>
    </div>
  </div>
  ...
  <div class="flex flex-col rounded-md bg-base-200 mb-5">
    <div class="flex sm:flex-row flex-col bg-base-100 m-2 mb-3 rounded-md">
      ...
      @if ($thread->title)
      <input id="title" value="{{ $thread->title }}" type="text" placeholder="Thread Title" class="input input-bordered input-sm sm:w-full sm:rounded-l-none" />
      @else
    ...
    <div class="block m-2.5">
      <textarea name="bbcode" id="bbcode-id" class="w-full h-80 sm:h-64">@if ($thread->threadComments->where('user_id','=', $thread->user_id)->first()->content){{ htmlspecialchars_decode($thread->threadComments->where('user_id','=', $thread->user_id)->first()->content) }}@endif</textarea>
    </div>
    @error('comment')
    <div class="text-red-500 text-sm">
      {{ $message }}
    ...
    <div class="flex justify-start items-center m-4">
      <button class="btn btn-sm" id="thread-update">
        <!-- edit -->
